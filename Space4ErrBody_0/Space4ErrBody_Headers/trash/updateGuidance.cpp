...
void MyGuidance::updateGuidance( const double currentTime )
{
    ...
    if( updateConditional < 1e-3 )
    {
        ...
        if ( FlightConditions_->getCurrentTime() != bislipSystems->getBankAngleReversalTimepoint() )
        {
            ...
            {
                if( debugInfo == 1 ){ std::cout << "               bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle ) * bislipSystems->getTempBankAngle( ) = "<< bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle ) * bislipSystems->getTempBankAngle( ) <<std::endl; }

                if ( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle ) * bislipSystems->getTempBankAngle( ) < 0.0 )
                {
    ...
    else
    {
        ...
        currentAngleOfAttack_ = bislipSystems->getCurrentValue( bislip::current_values::current_value_type::angle_of_attack );

        vehicleSystems->setCurrentControlSurfaceDeflection( "BodyFlap", bislipSystems->getCurrentValue( bislip::current_values::current_value_type::body_flap_deflection_angle) );
        vehicleSystems->setCurrentControlSurfaceDeflection( "ElevonLeft", bislipSystems->getCurrentValue( bislip::current_values::current_value_type::elevon_deflection_angle ) );
        vehicleSystems->setCurrentControlSurfaceDeflection( "ElevonRight", bislipSystems->getCurrentValue( bislip::current_values::current_value_type::elevon_deflection_angle ) );

        currentBankAngle_ = bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle );
        ...
        //    if ( currentTime == currentTime )
    {
        ...
        // bislipSystems->setCurrentValue( bislip::current_values::current_value_type::lift_magnitude, bislip::Variables::computeCurrentLiftForce( bodyMap_, vehicleName_ ) );

        // if( debugInfo == 1 ){ std::cout << "Calculate and Set Current Drag Force" << std::endl; }
        // bislipSystems->setCurrentValue( bislip::current_values::current_value_type::drag_magnitude, bislip::Variables::computeCurrentDragForce( bodyMap_, vehicleName_ ) );
    ...
    /*
    if( ( std::isnan( currentTime ) == false && bislipSystems->getCurrentValue( bislip::current_values::current_value_type::throttle_setting ) == 0.0 &&  bislipSystems->getCurrentEngineStatus() == true ) )
    {
        std::cout << "  " << std::endl;
        std::cout << "  " << std::endl;
        ...
        const double &timeOfFLight )
{
    ...
    if( bislipSystems->getInitialValueFlag() == true )
    {
        ...
        currentMachNumber      = bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::mach_number );
        currentHeight          = bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::height );
        currentAirspeed        = bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::airspeed);
        currentDynamicPressure = bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::dynamic_pressure);
        currentFlightPathAngle = tudat::unit_conversions::convertDegreesToRadians( bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::flight_path_angle ) );
        currentHeadingAngle    = tudat::unit_conversions::convertDegreesToRadians( bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::flight_path_angle ) );
        currentLatitude        = bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::latitude_angle);
        currentLongitude       = bislipSystems->getInitialValue( bislip::initial_values::initial_value_type::longitude_angle);
    ...
    else
    {
        ...
        previousBankAngle      = bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle );
    ...
    if( debugInfo == 1 ){ std::cout << "Starting Angle of Attack  = " << tudat::unit_conversions::convertRadiansToDegrees( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::angle_of_attack ) ) << std::endl; }
    if( debugInfo == 1 ){ std::cout << "Starting Bank Angle       = " << tudat::unit_conversions::convertRadiansToDegrees( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle ) ) << std::endl; }
    if( debugInfo == 1 ){ std::cout << " "<< std::endl; }
    ...
    /*if( bislipSystems->getCurrentEngineStatus( ) == true )
        {
            const double throttleDifference1 = std::abs( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::throttle_setting ) - ( bislipSystems->getThrottleSettingLimits() ).first );
            const double throttleDifference2 = std::abs( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::throttle_setting ) - ( bislipSystems->getThrottleSettingLimits() ).second );

            if( ( throttleDifference1 < -1e-7 || throttleDifference2 < -1e-7 ) )
            {
                //if( bodyFixedTotal_g_Load_Magnitude < bislipSystems->getAscentMechanicalLoadConstraint() )
                //{
                ...
                if( debugInfo == 1 ){ std::cout << "   Throttle Setting        = " << bislipSystems->getCurrentValue( bislip::current_values::current_value_type::throttle_setting ) << std::endl; }
                ...
                bislipSystems->setCurrentValue( bislip::current_values::current_value_type::angle_of_attack, optimizedAngleofAttack );
                //}
            }
    ...
    if( debugInfo == 1 ){ std::cout << "    Determined Angle of attack (interpolated)               = " << tudat::unit_conversions::convertRadiansToDegrees( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::angle_of_attack ) ) << std::endl; }
    ...
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::angle_of_attack, evaluatedAngleOfAttack );
    currentAngleOfAttack_ = bislipSystems->getCurrentValue( bislip::current_values::current_value_type::angle_of_attack );

    if( debugInfo == 1 ){ std::cout << "        Bounded angle of attack = " << bislipSystems->getCurrentValue( bislip::current_values::current_value_type::angle_of_attack ) << std::endl; }
    ...
    if( bislipSystems->getInitialValueFlag() == true )
    {
        ...
        controlSurfaceDeflections << bislipSystems->getCurrentValue( bislip::current_values::current_value_type::body_flap_deflection_angle), bislipSystems->getCurrentValue( bislip::current_values::current_value_type::elevon_deflection_angle ); }
    else
    {
    ...
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::body_flap_deflection_angle controlSurfaceDeflections( 0 ) );
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::elevon_deflection_angle, controlSurfaceDeflections( 1 ) );
   ...
   bislipSystems->setFullCurrentCoefficients( fullCurrentCoefficients );
    ...
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::lift_magnitude, bislip::Variables::computeCurrentLiftForce( bodyMap_, vehicleName_ ) );
    if( debugInfo == 1 ){ std::cout << "    Calculated Lift Force = " << bislipSystems->getCurrentValue( bislip::current_values::current_value_type::lift_magnitude) << std::endl; }

    if( debugInfo == 1 ){ std::cout << "Determine and set Drag Force" << std::endl; }

    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::drag_magnitude, bislip::Variables::computeCurrentDragForce( bodyMap_, vehicleName_ ) );
    if( debugInfo == 1 ){ std::cout << "    Calculated Drag Force = " << bislipSystems->getCurrentValue( bislip::current_values::current_value_type::drag_magnitude) << std::endl; }
    ...
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::bank_angle_limit bankAngleForSkipSuppression );
    ...
    if ( currentTrajectoryPhase == "Descent" )
    {
        ...
        if( currentDynamicPressure > bislipSystems->getMinimumDynamicPressureforControlSurface() )
        {
            ...
            bislipSystems->setCurrentValue( bislip::current_values::current_value_type::bank_angle,  tudat::unit_conversions::convertDegreesToRadians( bislip::Variables::evaluateGuidanceInterpolator( bislip::Parameters::Interpolators::BankAngle, bodyMap_, vehicleName_, centralBodyName_ ) ) );
            if( debugInfo == 1 ){ std::cout << "    Evaluated Bank Angle = " << bislipSystems->getCurrentValue( bislip::current_values::current_value_type::evaluated_bank_angle) << std::endl; }
            ...
            if( ( FlightConditions_->getCurrentTime() > startingEpoch_ + bislipSystems->getSkipSuppressionTimingTrigger( ) ) )
            {
                ...
                if( debugInfo == 1 ){ std::cout << "                    Current Bank Angle = " <<  tudat::unit_conversions::convertRadiansToDegrees( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle  ) ) << " deg  |  " << "Calculated Bank Angle: " <<  tudat::unit_conversions::convertRadiansToDegrees( bankAngle ) << " deg  |  " << "Bank Angle Required = " <<  tudat::unit_conversions::convertRadiansToDegrees( bankAngleForSkipSuppression ) << " deg" << std::endl; }
    ...
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::bank_angle bankAngle );
    currentBankAngle_ = bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle );
...
*/
    ...
    if ( currentTrajectoryPhase == "Descent" )
    {
        ...
        //if( debugInfo == 1 ){ std::cout << "timeOfFLight = " << timeOfFLight << "  |  " << "Evaluated Bank Angle = " << tudat::unit_conversions::convertRadiansToDegrees( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::evaluated_bank_angle) ) << "  ||  " << "Current Heading Error = " << currentHeadingAngleError << "  |  " << "Reversal Conditional = " << reversalConditional << "  |  " << " DeadBand = " << tudat::unit_conversions::convertDegreesToRadians( bislip::Variables::computeHeadingErrorDeadBand( bodyMap_, vehicleName_ ) ) << "  |  " << "Reversal = " << reversal << std::endl; }

        //std::cout << "timeOfFLight = " << timeOfFLight << "  |  " << "Evaluated Bank Angle = " << tudat::unit_conversions::convertRadiansToDegrees( bislipSystems->getCurrentValue( bislip::current_values::current_value_type::evaluated_bank_angle) ) << "  |  " << "Current Heading Error = " << currentHeadingAngleError << "  |  " << "Reversal Conditional = " << reversalConditional << "  |  " << " DeadBand = " << tudat::unit_conversions::convertDegreesToRadians( bislip::Variables::computeHeadingErrorDeadBand( bodyMap_, vehicleName_ ) ) << "  |  " << "Reversal = " << reversal << std::endl;
    ...
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::bank_angle,  evaluatedBankAngle );
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::bank_angle_limit skipSuppressionBankAngle );
    bislipSystems->setCurrentValue( bislip::current_values::current_value_type::bank_angle bankAngle );
    currentBankAngle_ = bislipSystems->getCurrentValue( bislip::current_values::current_value_type::bank_angle  );
    ...
    if( bislipSystems->getCurrentEngineStatus( ) == true )
    {
        ...
        if( debugInfo == 1 ){ std::cout << "    --------------------------" << std::endl; }
        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::throttle_setting,  bislip::Variables::determineThrottleSetting( bodyMap_, vehicleName_, centralBodyName_ ) );
        if( debugInfo == 1 ){ std::cout << "    --------------------------" << std::endl; }
        ...
        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_magnitude, currentThrustMagnitude );
        if( debugInfo == 1 ){ std::cout << "    --------------------------" << std::endl; }
        ...
        /*
        double dif = bislipSystems->getCurrentThrustMagnitude()*std::cos(currentAngleOfAttack_) - bislipSystems->getCurrentValue( bislip::current_values::current_value_type::drag_magnitude);
...
*/
        ...
        // bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_elevation_angle,  bislip::Variables::determineThrustElevationAngle( bodyMap_, vehicleName_, centralBodyName_ ) );
       bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_elevation_angle,  0.0 );


        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_azimuth_angle, 0.0 );
        // }
        // else
        // {
        ...
        if( debugInfo == 1 ){ std::cout << "    --------------------------" << std::endl; }
        bislipSystems->setCurrentBodyFixedThrustDirection( bislip::Variables::computeBodyFrameThrustUnitVector( bodyMap_, vehicleName_ ) );
    }
    else
    {
        if( debugInfo == 1 ){ std::cout << "    Engine if OFF" << std::endl; }
        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::throttle_setting,  0.0 );
        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_magnitude, 0.0 );
        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_elevation_angle,  0.0 );
        bislipSystems->setCurrentValue( bislip::current_values::current_value_type::thrust_azimuth_angle, 0.0 );
