...
for k = 1:(numel(pop_i) + 1)
    ...
    for p = 1:population
        ...
        evolutions(k).trajectories(p).individual.heading_angle         = rad2deg(convertNegativeAnglesToPositive(depvar(:,8)));
        ...
        evolutions(k).trajectories(p).individual.evaluated_bank_angle                 = rad2deg(depvar(:,36));
        ...
        evolutions(k).trajectories(p).individual.skip_suppression_limit               = rad2deg(depvar(:,49));
        ...
        evolutions(k).trajectories(p).individual.commanded_throttle_setting           = rad2deg(depvar(:,74));
        evolutions(k).trajectories(p).individual.commanded_thrust_elevation_angle     = rad2deg(depvar(:,75));
        evolutions(k).trajectories(p).individual.commanded_thrust_azimuth_angle       = rad2deg(depvar(:,76));
        evolutions(k).trajectories(p).individual.commanded_angle_of_attack            = rad2deg(depvar(:,77));
        evolutions(k).trajectories(p).individual.commanded_bank_angle                 = rad2deg(depvar(:,78));
        evolutions(k).trajectories(p).individual.currentLiftForce                     = depvar(:,79);
        evolutions(k).trajectories(p).individual.bank_angle_reversal_trigger          = depvar(:,end);
