...
for k = 1:(numel(pop_i) + 1)
    ...
    for p = 1:population
        ...
        evolutions(k).trajectories(p).individual.q_dot_LE              = rad2deg(depvar(:,42));
        evolutions(k).trajectories.individual.body_thrust_x           = depvar(:,43);
        evolutions(k).trajectories.individual.body_thrust_y           = depvar(:,44);
        evolutions(k).trajectories.individual.body_thrust_z           = depvar(:,45);
        evolutions(k).trajectories.individual.bending_moment          = depvar(:,46);
        evolutions(k).trajectories.individual.local_gravity_1         = depvar(:,47);
        evolutions(k).trajectories.individual.local_gravity_2         = depvar(:,48);
        evolutions(k).trajectories.individual.eq_glide_limit          = depvar(:,49);
        evolutions(k).trajectories.individual.increment_Cm_bodyflap   = depvar(:,50);
