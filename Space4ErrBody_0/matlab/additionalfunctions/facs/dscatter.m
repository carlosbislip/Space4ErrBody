...
if nargin > 2
    ...
    okargs = {'smoothing','bins','plottype','logy','aa','marker','msize','axes','filled','alpha'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
if isempty(nbins)
    nbins = [min(numel(unique(X)),6000) ,min(numel(unique(Y)),6000) ];
end
...
elseif length(k)>1
    ...
    switch(k)
        ...
        case 5 %'scatter'
            ...
            if filled
                if ishghandle(ax, 'axes') == 1
                    h = scatter(ax,X,Y,msize,col,marker,'filled','MarkerFaceAlpha',alpha);
                else
                    h = scatter(X,Y,msize,col,marker,'filled','MarkerFaceAlpha',alpha);
                end
            else
                if ishghandle(ax, 'axes') == 1
                    h = scatter(ax,X,Y,msize,col,marker,'MarkerFaceAlpha',alpha);
                else
                    h = scatter(X,Y,msize,col,marker,'MarkerFaceAlpha',alpha);
                end
            end
    end
