function align_axislabel_log(~,ax)
...
% Feb/06/2015
% Apr/10/2016
% Dec/28/2016, Adds support for log scale
...
base = 10;
% islog_x = strcmpi(get(h_a, 'XScale'), 'log');
% islog_y = strcmpi(get(h_a, 'YScale'), 'log');
islog_z = strcmpi(get(h_a, 'ZScale'), 'log');
...
if proj_mode == 1
    ...
    switch ix % use ix to determine z axis, then use iy to determine x and y axis
        case {1,5}
            trans_pnt_z = [x_lim(1),y_lim(1),...
                find_middle_point([z_lim(1) z_lim(2)], islog_z, base)]';
            if iy == 2
                trans_vec_x_edge = pnt_cam(1:2,2)-pnt_cam(1:2,3);
                trans_vec_y_edge = pnt_cam(1:2,2)-pnt_cam(1:2,1);
                trans_pnt_y = (pnt(1:3,2)+pnt(1:3,1))/2;
                trans_pnt_x = (pnt(1:3,3)+pnt(1:3,2))/2;
            else % iy == 4
...
function m = find_middle_point(pnts, islog, base)
if islog
    log_pnt = (log(pnts(1))+log(pnts(2)))/log(base);
    m = base^(log_pnt/2);
else
    m = (pnts(1)+pnts(2))/2;
end
