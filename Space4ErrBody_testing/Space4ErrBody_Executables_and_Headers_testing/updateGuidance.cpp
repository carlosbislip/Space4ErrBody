...
void MyAerodynamicGuidance::updateGuidance( const double currentTime )
{
    ...
    const double new_gamma = getGamma( A, B, phi );
    ...
    //! for the current one.
    const double eps_T = getEps_T( AoA, p, newCoefficients );
